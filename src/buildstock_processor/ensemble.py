"""Ensemble ("mixed-use") building representation for BuildStock.
...
building type -- for example, a building that is 70% office space over 30% ground-floor retail. An
`EnsembleBuildingType` models such a building as a weighted combination of one or more BuildStock
`(product, building_type)` components, each contributing a `fraction` of the combined building's energy
profile. Fractions across all components must sum to 1.0 (see `EnsembleBuildingType.normalized()` if your
shares don't sum perfectly due to rounding).

This module provides two levels of functionality:

- `combine_ensemble_time_series()` combines already-downloaded per-component time series DataFrames (e.g.
  from `BuildStockProcessor.process_building_time_series()`) into one synthetic ensemble time series,
  linearly blended by each component's `fraction`.
- `pull_ensemble_time_series()` is the end-to-end version: for each component, builds the right processor,
  finds (or uses a caller-supplied) representative building, downloads its time series, then combines
  everything with `combine_ensemble_time_series()`.
...
so ensembles can freely mix ComStock and ResStock components (e.g. ground-floor retail over apartments).

`fraction` is unrelated to the `weight` metadata column (which scales a sampled building/unit up to the
number of real buildings/units it represents in the overall stock) -- it's simply how much of the ensemble
...
class EnsembleComponent:
    """One weighted BuildStock building-type component of an `EnsembleBuildingType`."""
    ...
    fraction: float
    """This component's share of the combined ensemble profile, in (0, 1]. All of an ensemble's component
    fractions must sum to 1.0."""

    def __post_init__(self) -> None:
        if not 0 < self.fraction <= 1:
            raise ValueError(f"EnsembleComponent fraction must be in (0, 1], got {self.fraction}")
        normalized_product = self.product.strip().lower()
        if normalized_product not in {"comstock", "resstock"}:
            raise ValueError(f"EnsembleComponent product must be 'comstock' or 'resstock', got {self.product!r}")
...
class EnsembleBuildingType:
    ...
    def __post_init__(self) -> None:
        if len(self.components) < 2:
            raise ValueError("An EnsembleBuildingType needs at least 2 components")
        keys = [component.key for component in self.components]
        if len(set(keys)) != len(keys):
            raise ValueError(f"Ensemble components must have unique (product, building_type) pairs, got {keys}")
        # Note: fractions summing to 1.0 is intentionally *not* enforced here -- see `assert_normalized()`,
        # which `combine_ensemble_time_series()`/`pull_ensemble_time_series()` call before combining. That
        # keeps `normalized()` usable on a freshly constructed, not-yet-normalized ensemble.
    ...
    def assert_normalized(self) -> None:
        """Raise `ValueError` unless this ensemble's component fractions sum to ~1.0."""
        if not math.isclose(self.total_fraction, 1.0, abs_tol=_WEIGHT_SUM_TOLERANCE):
            raise ValueError(
                f"Ensemble '{self.name}' component fractions must sum to 1.0 (got {self.total_fraction}); "
                "rescale them or call `.normalized()` if they're percentages that don't sum perfectly due to rounding."
            )

    @classmethod
    def from_fractions(cls, name: str, fractions: Mapping[tuple[str, str], float]) -> EnsembleBuildingType:
        """Convenience constructor from a `{(product, building_type): fraction}` mapping."""
        return cls(
            name=name,
            components=tuple(
                EnsembleComponent(product=product, building_type=building_type, fraction=fraction)
                for (product, building_type), fraction in fractions.items()
            ),
        )

    def normalized(self) -> EnsembleBuildingType:
        ...
        return EnsembleBuildingType(
            name=self.name,
            components=tuple(
                EnsembleComponent(product=component.product, building_type=component.building_type, fraction=component.fraction / total)
                for component in self.components
            ),
        )

    def component_map(self) -> dict[tuple[str, str], EnsembleComponent]:
        """Return this ensemble's components keyed by `(product, building_type)`."""
...
def combine_ensemble_time_series(
    ensemble: EnsembleBuildingType,
    component_time_series: Mapping[tuple[str, str], pd.DataFrame],
    value_columns: list[str] | None = None,
    timestamp_column: str = "timestamp",
) -> pd.DataFrame:
    """Linearly combine each component's time series DataFrame into one synthetic ensemble time series.

    `component_time_series` must have one entry per `ensemble` component, keyed by
    `EnsembleComponent.key` (i.e. `(product, building_type)`), each a DataFrame like the ones returned by
    ...
    required_keys = [component.key for component in ensemble.components]
    missing = [key for key in required_keys if key not in component_time_series]
    if missing:
        raise ValueError(f"Missing component time series for ensemble '{ensemble.name}': {missing}")
    ...
    if not value_columns:
        raise ValueError("No shared numeric columns found across ensemble components to combine.")
    ...
    if len(shared_index) == 0:
        raise ValueError(f"Component time series for ensemble '{ensemble.name}' share no common timestamps to combine.")
...
def pull_ensemble_time_series(
    ...
    """End-to-end: download one representative building's time series per ensemble component, then combine
    them into a single synthetic ensemble time series.
    ...
    Args:
        ensemble: the `EnsembleBuildingType` to pull and combine time series for.
        ...
        value_columns: passed through to `combine_ensemble_time_series()`.

    Returns:
        A tuple of `(combined_ensemble_time_series, {component.key: component_time_series})` so callers can
    ...
    for component in ensemble.components:
        ...
        if bldg_id is not None:
            ...
            if metadata.empty:
                raise ValueError(f"No buildings found for ensemble component {component.key} in state={state!r}.")
        ...
        if not paths:
            raise ValueError(f"Failed to download time series for ensemble component {component.key}.")
        component_time_series[component.key] = pd.read_parquet(paths[0])

    combined = combine_ensemble_time_series(ensemble, component_time_series, value_columns=value_columns)
