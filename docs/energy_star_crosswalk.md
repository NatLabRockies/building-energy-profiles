...
This file mirrors `src/building_energy_profiles/energy_star_crosswalk.json` in a Markdown format that is easy for people and simple tools to parse. The Python API exposes the same data through `energy_star_crosswalk()`, `map_energy_star_property_type()`, and `energy_star_property_types_for_buildstock_type()`.
